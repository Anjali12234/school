        ...
        <div class="card-box mb-30"  style="min-height: 70vh;">
            <div class="pd-20">
                ...
                <div class="mb-3 float-right justify-content-end">
                    <button class="btn btn-secondary"><a class="text-white " href="{{ route('admin.service.create') }}">Add
                            +</a></button>
                    ...
                    <tbody>
                        @forelse ($services as $key=>$service)
                            <tr>
                                ...
                                <td>{{ $service->title }}</td>
                                <td><img src="{{ $service->image }}" alt="" width="100px"></td>
                                <td>
                                    <form action="{{ route('admin.service.updateServiceStatus', $service) }}" method="post"
                                        style="display: inline">
                                        @csrf
                                        @method('put')
                                        <button type="submit" style="border: none; background: none;">
                                            <i
                                                class="fa fa-{{ $service->status == 1 ? 'toggle-on text-success' : 'toggle-off text-danger' }} fa-2x"></i>
                                        </button>
                                    </form>
                                </td>
                                <td>
